...
# - added attrs() to print info
# - add polarization AA+BB for multi-pol
...
class PSRFITS:
    ...
    def attrs(self):
        """Print attributes"""
        print("Source info ...")
        print(f"\tSource: {self.srcname}")
        print(f"\tNpol: {self.npol}")
        print(f"\tNbit: {self.nbits}")
        print(f"\tNchan: {self.nchan}")
        print(f"\tNsblk: {self.nsblk}")
    ...
    def getData(self, start_row=0, end_row=None, downsamp=1, fdownsamp=1, apply_scales=False, get_ft=False, squeeze=False, transpose=False):
        ...
        for irow in range(nrows_tot):
            ...
            if DEBUG:
                print(f"DIM of fits['SUBINT']['DATA']={dtmp.shape}, NSBLK={nsblk_ds}")

            # Unpack data if data is still packed
            if nsblk_ds//dtmp.shape[0] != 1:
                if self.nbits == 1:
                    dtmp = numpy.unpackbits(dtmp, axis=0, bitorder='big')
                else:
                    # Unpack and repack data to nbits
                    dtmp = numpy.unpackbits(dtmp, bitorder='big').reshape(-1, self.nbits)[...,::-1]
                    dtmp = numpy.packbits(dtmp, axis=1, bitorder='little').reshape(nsblk_ds, self.npol, nchan_ds)
                if DEBUG:
                    print(f"{self.nbits}-bit data are initially packed, but are unpacked")
        ...
        if self.npol > 1:
            print("\tAdding AA+BB polarization")
            result = result[:, 0, numpy.newaxis, :] + result[:, 1, numpy.newaxis, :] # Add newaxis for dimension [NSBLK, NPOL, NCHAN]
